...
monthly_tables = [['2017-04-week-1', '2017-04-week-2', '2017-04-week-3', '2017-04-week-4']]
...
for monthly_table in monthly_tables:
    ...
    count.to_csv(weekly_table+'_group_by_song.csv', index=True)
    
    count = monthly_result.groupby(['artist'])['artist'].count().reset_index(name='Count').sort_values(['Count'], ascending=False)
    count.to_csv(weekly_table+'_group_by_artist.csv', index=True)
